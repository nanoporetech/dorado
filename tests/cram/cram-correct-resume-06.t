Test a skip set composed of 4 reads, and a proper default index-size of 8GB as is in production.
  ...
  > cat ${in_reads}.fai | cut -f 1,1 | head -n 4 > skip_set.txt
  > ${DORADO_BIN} correct ${in_reads} -t 4 ${model_var} --resume-from skip_set.txt > out/out.fasta 2> out/out.fasta.stderr
