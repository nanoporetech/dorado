Stereo models should fail.
  ...
  > samtools view -h ${in_dir}/in.aln.bam | sed 's/dna_r10.4.1_e8.2_400bps_hac@v5.0.0/dna_r10.4.1_e8.2_5khz_stereo@v1.3/g' | samtools view -Sb > out/in.bam
  > samtools index out/in.bam
  > ### Run the unit under test.
  > in_bam=out/in.bam
  > in_ref=${in_dir}/in.ref.fasta.gz
  > in_expected=${in_dir}/expected.dorado.vcf
  > ${DORADO_BIN} variant --device cpu ${in_bam} ${in_ref} -t 4 --ignore-read-groups > out/out.vcf 2> out/stderr
  > ### Eval.
  > echo "Exit code: $?"
  > grep "\[warning\]" out/stderr | sed -E 's/.*(\[warning\].*)/\1/g'
  > grep "\[error\]" out/stderr | sed -E 's/.*(\[error\].*)/\1/g'
  Exit code: 1
...
This still fails because the model cannot be resolved automatically.
  ...
  > in_ref=${in_dir}/in.ref.fasta.gz
  > in_expected=${in_dir}/expected.dorado.vcf
  > ${DORADO_BIN} variant --skip-model-compatibility-check --device cpu ${in_bam} ${in_ref} -t 4 --ignore-read-groups > out/out.vcf 2> out/stderr
  > ### Eval.
  > echo "Exit code: $?"
  > grep "\[warning\]" out/stderr | sed -E 's/.*(\[warning\].*)/\1/g'
  > grep "\[error\]" out/stderr | sed -E 's/.*(\[error\].*)/\1/g'
...
This succeeds because the model was explicitly specified.
  ...
  > ${DORADO_BIN} variant --skip-model-compatibility-check --device cpu ${in_bam} ${in_ref} -t 4 ${model_var} --ignore-read-groups > out/out.vcf 2> out/stderr
  > ### Eval.
  > echo "Exit code: $?"
  > grep "\[warning\]" out/stderr | sed -E 's/.*(\[warning\].*)/\1/g'
  > grep "\[error\]" out/stderr | sed -E 's/.*(\[error\].*)/\1/g'
