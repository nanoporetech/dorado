Unknown model.
  ...
  > model="unknown"
  > ${DORADO_BIN} polish --device cpu --window-len 1000 --window-overlap 100 --bam-chunk 2000 ${in_bam} ${in_draft} -t 4 --infer-threads 1 --model "${model}" -vv > out/out.fasta 2> out/stderr
  > echo "Exit code: $?"
...
Download a model by name.
  ...
  > ${DORADO_BIN} polish --device cpu --window-len 1000 --window-overlap 100 --bam-chunk 1000000 ${in_bam} ${in_draft} -t 4 -vv --model "${model}" > out/out.fasta 2> out/stderr
  > echo "Exit code: $?"
