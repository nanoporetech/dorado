Test index-size equal to 1bp. This should make every single read as an individual chunk.
  $ rm -rf out; mkdir -p out
  > in_reads=${TEST_DATA_DIR}/read_correction/reads.fq
  > model_var=${MODEL_DIR:+--model-path ${MODEL_DIR}}
  > ${DORADO_BIN} correct ${in_reads} -t 4 ${model_var} -v --index-size 0 > out/out.fasta 2> out/out.fasta.stderr
