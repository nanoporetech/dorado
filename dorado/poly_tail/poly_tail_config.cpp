...
const std::string STATUS_TOML_KEY{"status"};
...
PolyTailConfig update_config(const toml::value& config_toml, PolyTailConfig config) {
    config.enabled = true;
    if (config_toml.contains(STATUS_TOML_KEY)) {
        const auto& status_toml = toml::find(config_toml, STATUS_TOML_KEY);
        if (status_toml.contains("enabled")) {
            config.enabled = toml::find<bool>(status_toml, "enabled");
        }
    }
    ...
    if (config_toml.contains(ANCHORS_TOML_KEY)) {
    ...
    if (config_toml.contains(THRESHOLD_TOML_KEY)) {
    ...
    if (config_toml.contains(TAIL_TOML_KEY)) {
