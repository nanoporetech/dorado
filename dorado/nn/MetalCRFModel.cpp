...
struct MetalConv1dImpl : Module {
    ...
    void run(MTL::CommandQueue *command_queue, MTL::Buffer *mat_in, MTL::Buffer *mat_out) {
        ...
        launch_kernel(conv_cps, command_queue, buffers, kernel_thread_groups, kernel_simd_groups * 32);
    }

    void run(MTL::CommandBuffer *command_buffer, MTL::Buffer *mat_in, MTL::Buffer *mat_out) {
        std::vector<MTL::Buffer *> buffers{args, mat_in, mat_weights, mat_out};
        launch_kernel_no_wait(conv_cps, command_buffer, buffers, kernel_thread_groups, kernel_simd_groups * 32);
    }
...
struct MetalLSTMImpl : Module {
    ...
    MetalLSTMImpl(int layer_size, bool reverse_, MTL::Device *device) : reverse(reverse_) {
        ...
        mat_weights = create_buffer(device, (size_t) (layer_size * 2 + 1) * layer_size * 4 * sizeof(ftype));
    }
...
struct MetalBlockImpl : Module {
    ...
    : device(device_), in_chunk_size(chunk_size_), batch_size(batch_size_), layer_size(layer_size_), out_size(out_size_) {
        ...
        fn[0] = "lstm_simd_" + std::to_string(layer_size) + "_fwd_" + std::to_string(kernel_simd_groups);
        fn[1] = "lstm_simd_" + std::to_string(layer_size) + "_rev_" + std::to_string(kernel_simd_groups);
        std::string linear_fn = "linear_tanh_simd_" + std::to_string(layer_size) + "_fwd_" + std::to_string(kernel_simd_groups);
        ...
        mat_transfer = create_buffer(device, (size_t) batch_size * lstm_chunk_size * out_size * sizeof(float));
        mat_transfer_ftype = (sizeof(ftype) == 4) ? mat_transfer : create_buffer(device, (size_t) batch_size * in_chunk_size * sizeof(ftype));
        mat_working_mem = create_buffer(device, size_t(lstm_chunk_size + 2) * batch_size * layer_size * sizeof(ftype));
    ...
    void load_weights() {
        ...
        for (auto& rnn : {rnn1, rnn2, rnn3, rnn4, rnn5}) {
            ...
            std::vector<MTL::Buffer *> buffers{args[rnn->reverse], mtl_for_tensor(t_w), mtl_for_tensor(t_u), mtl_for_tensor(t_b), rnn->mat_weights};
        ...
        if (sizeof(ftype) != sizeof(float)) {
            ...
            launch_kernel(to_half_cps, command_queue, {args, mtl_for_tensor(t_w), mat_linear_weights}, kernel_thread_groups, 256);
        }
        else {
    ...
    std::pair<MTL::CommandBuffer*, torch::Tensor> forward_async(torch::Tensor x) {
        ...
        if (sizeof(ftype) == 2) {
            launch_kernel_no_wait(to_half_cps, command_buffer, {args[2], mtl_for_tensor(x), mat_transfer_ftype}, kernel_thread_groups, 256);
        ...
        for (auto& rnn : {rnn1, rnn2, rnn3, rnn4, rnn5}) {
            std::vector<MTL::Buffer *> buffers{args[rnn->reverse], mat_working_mem, rnn->mat_weights, mat_state, mat_temp_result};
            launch_kernel_no_wait(lstm_cps[rnn->reverse], command_buffer, buffers, kernel_thread_groups, kernel_simd_groups * 32);
        }
