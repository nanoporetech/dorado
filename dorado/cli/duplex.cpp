...
int duplex(int argc, char* argv[]) {
    ...
    if (model.compare("basespace") == 0) {
        ...
        BaseSpaceDuplexCallerNode duplex_caller_node(writer_node, template_complement_map, read_map, threads);
        ...
        if (device == "cpu") {
            ...
            for (size_t i = 0; i < num_runners  ; i++) {
                runners.push_back(std::make_shared<ModelRunner<CPUDecoder>>(model, device,
        ...
        StereoDuplexEncoderNode stereo_node = StereoDuplexEncoderNode(writer_node); //Currently the StereoDuplexEncoderNode just outputs the read it receives into the writer node
