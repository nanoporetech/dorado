...
//  (and what we construct for the new timestep)
    ...
    bool score_sort(const BeamFrontElement& a, const BeamFrontElement& b) {
    ...
    std::tuple<std::string, std::string> generate_sequence(
...
float beam_search(const T* scores, const T* back_guide, const float* posts,
    ...
    if (max_beam_width < num_states) {
        ...
        std::nth_element(sorted_back_guides.begin(), sorted_back_guides.begin() + max_beam_width - 1,
                         sorted_back_guides.end(), std::greater<T>());
    ...
    for (size_t state = 0, beam_element = 0; state < num_states && beam_element < max_beam_width; state++) {
        if (back_guide[state] >= beam_init_threshold) {
            ...
            (*prev_beam_front)[beam_element++] =
                    { chainfasthash64(hash_seed, state), 0.0f, state_t(state), 0, false };
        }
    }
	 ...
	 */
	auto generate_move_index = [](state_t previous_state, state_t new_state, size_t num_bases, size_t num_states) {
        ...
        for (size_t prev_elem_idx = 0; prev_elem_idx < current_beam_width; prev_elem_idx++) {
            ...
            for (size_t new_base = 0; new_base < num_bases; new_base++) {
                state_t new_state = state_t((previous_element.state * num_bases) % num_states + new_base);
                const state_t move_idx = generate_move_index(previous_element.state, new_state, num_bases, num_states);
                float new_score = previous_element.score + float(block_scores[move_idx]) + float(block_back_scores[new_state]);
                ...
                (*current_beam_front)[new_elem_count++] =
                        { new_hash, new_score, new_state, (uint8_t)prev_elem_idx, false};
            }
        }
...
#endif
            (*current_beam_front)[new_elem_count++] =
                    { previous_element.hash, stay_score, previous_element.state, (uint8_t)prev_elem_idx, true };
        }
        ...
        for (size_t prev_elem_idx = 0; prev_elem_idx < current_beam_width; prev_elem_idx ++) {
            ...
            for (size_t prev_elem_comp_idx = 0; prev_elem_comp_idx < current_beam_width; prev_elem_comp_idx++) {
                ...
                if ((*current_beam_front)[stay_elem_idx].hash == (*current_beam_front)[step_elem_idx].hash) {
                    if ((*current_beam_front)[stay_elem_idx].score > (*current_beam_front)[step_elem_idx].score) {
                        // Fold the step into the stay
                        (*current_beam_front)[stay_elem_idx].score =
                                log_sum_exp((*current_beam_front)[stay_elem_idx].score,
                                            (*current_beam_front)[step_elem_idx].score, temperature);
                        // The step element will end up last, sorted by score
                        (*current_beam_front)[step_elem_idx].score = -std::numeric_limits<float>::max();
                    } else {
                        ...
                        (*current_beam_front)[stay_elem_idx].score = -std::numeric_limits<float>::max();
                    }
                }
            }
        }
        ...
        for (size_t elem_idx = 0; elem_idx < new_elem_count; elem_idx ++) {
            if ((*current_beam_front)[elem_idx].score > max_score) max_score = (*current_beam_front)[elem_idx].score;
        }

        // Starting point for finding the cutoff score is the beam cut score
        float beam_cutoff_score = max_score - log_beam_cut;

        auto get_elem_count = [](const std::vector<BeamFrontElement> *current_beam_front, size_t new_elem_count, float beam_score) {
            // Count the elements which meet the beam score
            size_t count = 0;
            for (size_t elem_idx = 0; elem_idx < new_elem_count; elem_idx ++) {
                if ((*current_beam_front)[elem_idx].score >= beam_score) count++;
            }
            return count;
        };
    ...
    for (size_t block_idx = 0; block_idx < num_blocks; block_idx++) {
        ...
        bool is_hp = state == hp_states[0] || state == hp_states[1]
                     || state == hp_states[2] || state == hp_states[3];
...
std::tuple<std::string , std::string, std::vector<uint8_t>> beam_search_decode(
        const torch::Tensor scores_t, const torch::Tensor back_guides_t, const torch::Tensor posts_t,
        size_t beam_width, float beam_cut, float fixed_stay_score, float q_shift, float q_scale, float temperature
) {
    ...
    if (type_str == "float") {
        ...
        throw std::runtime_error(std::string("beam_search_decode: unsupported tensor type ") + type_str);
    }
