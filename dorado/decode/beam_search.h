...
template <typename T>
void merge_sort(T* data, const size_t count, const size_t cutoff, bool(*less_func)(const T&, const T&)) {
    ...
    for (size_t src_block_size = 1; src_block_size < count; src_block_size *= 2) {
        ...
        for (size_t start_idx = 0; start_idx < count; start_idx += src_block_size * 2) {
            ...
            size_t end_merge_idx = std::min(count, start_idx + src_block_size * 2); // idx of to element after the last one we should merge
            ...
            for (T* dest = dest_buff + start_idx; dest < dest_buff + end_write_idx; dest++) {
                if (first_blk == second_blk_start // All of first block consumed
                    || (second_blk < source_buff + end_merge_idx && less_func(*second_blk, *first_blk))) { // second block available and should go first
...
beam_search_decode(const torch::Tensor scores_t,
