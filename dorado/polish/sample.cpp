...
Sample slice_sample(const Sample& sample, const int64_t idx_start, const int64_t idx_end) {
    ...
    if ((idx_start < 0) || (idx_start >= num_columns) || (idx_end >= idx_start) || (idx_end > num_columns)) {
    ...
    if ((dorado::ssize(sample.positions_minor) != num_columns) ||
        (sample.depth.size(0) != num_columns) ||
        (sample.features.size(0) != num_columns)) {
        throw std::invalid_argument("Input data dimensions are inconsistent. num_columns = " + std::to_string(num_columns) +
            ", sample.positions_minor.size = " + std::to_string(std::size(sample.positions_minor)) +
    ...
    // Slice vector data
    sliced_sample.positions_major = std::vector<int64_t>(
        std::begin(sample.positions_major) + idx_start,
        std::begin(sample.positions_major) + idx_end);
    sliced_sample.positions_minor = std::vector<int64_t>(
        std::begin(sample.positions_minor) + idx_start,
