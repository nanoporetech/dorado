...
MetalModelImpl::MetalModelImpl(const CRFModelConfig &config,
                               int chunk_size,
                               int batch_size,
                               int out_split,
                               MTL::Device *const device) {
...
void MetalModelImpl::load_state_dict(const std::vector<at::Tensor> &weights) {
    utils::load_state_dict(*this, weights, {});
    mtl_block->load_weights();
}

MTL::CommandBuffer *MetalModelImpl::forward_async(at::Tensor &in,
                                                  MTL::SharedEvent *const linear_hold_off_event,
                                                  uint64_t linear_hold_off_id,
                                                  int try_count,
                                                  std::vector<at::Tensor> &out) {
