...
struct TxEncoderImpl : torch::nn::Module {
    TxEncoderImpl(const config::tx::TxEncoderParams &params, const at::TensorOptions &options);

    at::Tensor forward(at::Tensor x);

    void koi_forward(utils::ScaledTensor &scaled_tensor, at::Tensor &x_f16);

    config::tx::TxEncoderParams params;
...
struct TxEncoderStackImpl : torch::nn::Module {
    TxEncoderStackImpl(const config::tx::TxEncoderParams &params, const at::TensorOptions &options);
...
struct LinearUpsampleImpl : torch::nn::Module {
    LinearUpsampleImpl(const config::tx::EncoderUpsampleParams &params);
...
struct LinearScaledCRFImpl : torch::nn::Module {
    LinearScaledCRFImpl(const config::tx::CRFEncoderParams &params);

    at::Tensor forward(const at::Tensor &x);

    bool scale_applied = false;
    torch::nn::Linear linear{nullptr};
    config::tx::CRFEncoderParams m_params;
};
