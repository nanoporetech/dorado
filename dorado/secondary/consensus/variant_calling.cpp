...
/**
 * \brief Converts a sequence of bases (e.g. '*ACGT') to a numeric representation (e.g. [0,1,2,3,4]).
 */
std::vector<int32_t> encode_seq(const std::array<int32_t, 256>& symbol_lookup,
...
std::vector<Variant> decode_variants(const DecoderBase& decoder,
    ...
    const auto compute_seq_quality = [](const std::array<int32_t, 256>& symbol_lookup,
                                        const at::Tensor& class_probs, const std::string_view seq,
                                        const bool substitute_n) {
    ...
    if ((std::size(c) != 1) || (std::size(c.front()) != 1)) {
        spdlog::warn(
                ...
                "c.size = {}. Skipping variant decoding of this sample.",
                std::size(c));
        return {};
    }
    ...
    for (const auto& [rstart, rend, is_var] : runs) {
        ...
        const std::vector<std::pair<std::string, std::string>> genotype{
                {"GT", "1"},
                {"GQ", std::to_string(qual_i)},
        };
