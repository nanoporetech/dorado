...
std::shared_ptr<ModelTorchBase> model_factory(const ModelConfig& config,
    ...
    if (config.model_file == "model.pt") {
        ...
        model = std::make_shared<ModelTorchScript>(config.model_dir / config.model_file);
    ...
    if (model_type == ModelType::GRU) {
        ...
        model = std::make_shared<ModelGRU>(num_features, num_classes, gru_size, n_layers,
                                           bidirectional);
        ...
        model = std::make_shared<ModelLatentSpaceLSTM>(
        ...
        model = std::make_shared<ModelSlotAttentionConsensus>(
