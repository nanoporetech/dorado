...
if(CMAKE_SYSTEM_NAME STREQUAL "Linux" OR WIN32)
    ...
    elseif (CUDAToolkit_VERSION VERSION_LESS 14.0)
        # CUDA 12 and 13 doesn't support C++23.
    ...
    if(CMAKE_SYSTEM_NAME STREQUAL "Linux")
        if(CMAKE_SYSTEM_PROCESSOR MATCHES "^aarch64*|^arm*")
            if(CUDAToolkit_VERSION VERSION_GREATER_EQUAL 13.0)
                set(TORCH_PATCH_SUFFIX "3-ont-CUDA-13.0-linux-aarch64")
                set(TORCH_HASH "050c09a44cb800b5aa95f4f210fce53304c4ab862e05b103e800b9c6789a371a")
            else()
                set(TORCH_PATCH_SUFFIX "0-ont-CUDA-12.6-linux-aarch64")
        ...
        elseif(CUDAToolkit_VERSION VERSION_GREATER_EQUAL 13.0)
            set(TORCH_PATCH_SUFFIX "0-ont-CUDA-13.0-linux-x64-cxx11-abi")
        ...
        elseif(CUDAToolkit_VERSION VERSION_GREATER_EQUAL 12.8)
            set(TORCH_PATCH_SUFFIX "0-ont-CUDA-12.8-linux-x64-cxx11-abi")
            set(TORCH_HASH "ed09249f48bc3e20130307e86a729e8bc8a97aed794c149b60efcae172654a9d")
        else()
            set(TORCH_VERSION 2.7.1)
            set(TORCH_PATCH_SUFFIX "0-ont-CUDA-11.8-linux-x64-cxx11-abi")
            set(TORCH_HASH "6fbb32feb5311ffbb8d540118a099bae61ec9693b994d24687a9748b8d658e78")
        endif()
    elseif(APPLE)
        set(TORCH_PATCH_SUFFIX "2-ont-macos-m1")
        set(TORCH_HASH "84113189187bc3d3f4d3948a15b5c4ee13f05ffc4df1449e733e3dcf286e1c80")
    elseif(WIN32)
        if(CUDAToolkit_VERSION VERSION_GREATER_EQUAL 13.0)
            set(TORCH_PATCH_SUFFIX "0-ont-CUDA-13.0-windows")
            set(TORCH_HASH "4c3d50234fb871811c758edd1e310e55d94619e6e9764eb6ca3e00d0177e3330")
        else()
            set(TORCH_PATCH_SUFFIX "0-ont-CUDA-12.8-windows")
            set(TORCH_HASH "44219daaeafd0a7f194daede50783cd335b8d3d9c7bcc95ddb3dcfaec30a4a30")
        endif()
    endif()

    # Get libtorch (if we don't already have it)
    set(TORCH_LIB_DIR torch-${TORCH_VERSION}.${TORCH_PATCH_SUFFIX})
    set(TORCH_URL ${DORADO_CDN_URL}/torch-${TORCH_VERSION}.${TORCH_PATCH_SUFFIX}.zip)
    download_and_extract(${TORCH_URL} ${TORCH_LIB_DIR} ${TORCH_HASH})
    set(TORCH_LIB "${DORADO_3RD_PARTY_DOWNLOAD}/${TORCH_LIB_DIR}/libtorch")
endif()
